...
def load_data(filename):
    data = load(filename)
    split_data = split_dataframe_on_traffic_light_change(data)
    first_rows = []
    control_inputs = []
    outputs = []
    Bin_level_input = []
    for i in range(len(split_data)):
        columns_to_ignore = ['traffic_light', 'feeder 1st Motor','feeder 2st Motor','Bin Level']
        columns_to_convert = ['traffic_light', 'feeder 1st Motor','feeder 2st Motor', 'Bin Level']
        first_row, tensors, bin_level = create_tensors_from_dataframe(split_data[i],columns_to_ignore,columns_to_convert, 'Bin Level')
        first_rows.append(first_row)
        Bin_level_input.append(bin_level[:-1])
        tensor = torch.cat((tensors['traffic_light'].unsqueeze(0), tensors['feeder 1st Motor'].unsqueeze(0), tensors['feeder 2st Motor'].unsqueeze(0)), dim = 0)
        control_inputs.append(tensor)
        outputs.append(bin_level[1:])
...
def create_train_test(tensor_list):
    ...
    test = tensor_list[split_index:int(len(tensor_list) * 0.075)]  # 40%
    return train, test
